...
export const ListboxLabel = defineComponent({
  ...
  setup() {
    const api = useListboxContext('ListboxLabel')
    const id = `raxui-listbox-label-${useId()}`
...
export const ListboxButton = defineComponent({
  ...
  render() {
    ...
    const propsWeControl = {
      ...
      'aria-expanded':
        api.listboxState.value === ListboxStates.Open ? true : undefined,
  ...
  setup(props) {
    ...
    function handleFocus() {
      if (api.listboxState.value === ListboxStates.Open)
        return api.optionsRef.value?.focus()
      focused.value = true
    }
...
export const ListboxOptions = defineComponent({
  ...
  setup() {
    const api = useListboxContext('ListboxOptions')
    const id = `raxui-listbox-options-${useId()}`
...
export const ListboxOption = defineComponent({
  ...
  setup(props, { slots, attrs }) {
    ...
    const id = `raxui-listbox-option-${useId()}`
    const {
      disabled,
      class: defaultClass,
    ...
    const dataRef = ref<ListboxOptionDataRef['value']>({
      disabled,
      value,
      textValue: '',
    })
