...
function renderTemplate(
  input: string | Partial<Parameters<typeof defineComponent>[0]>
) {
  const defaultComponents = { Menu, MenuButton, MenuItems, MenuItem }

  if (typeof input === 'string') {
    return render(
      defineComponent({ template: input, components: defaultComponents })
    )
  }
...
describe('Safe guards', () => {
  ...
  it('should be possible to render a Menu without crashing', () => {
    ...
    assertMenuButton({
      ...
      attributes: { id: 'raxui-menu-button-1' },
    })
...
describe('Rendering', () => {
  ...
  describe('MenuButton', () => {
    ...
    it('should be possible to render a MenuButton using a template `as` prop', async () => {
      ...
      assertMenuButton({
        state: MenuState.InvisibleUnmounted,
        attributes: { id: 'raxui-menu-button-1', 'data-open': 'false' },
      })
      assertMenu({ state: MenuState.InvisibleUnmounted })
      ...
      assertMenuButton({
        state: MenuState.Visible,
        attributes: { id: 'raxui-menu-button-1', 'data-open': 'true' },
      })
      assertMenu({ state: MenuState.Visible })
    })
  ...
  it(
    ...
    suppressConsoleLogs(async () => {
      ...
      items.forEach((item) =>
        assertMenuItem(item, {
          tag: 'button',
          attributes: { 'data-my-custom-button': 'true' },
        })
      )
    })
  )
...
describe('Keyboard interactions', () => {
  describe('`Enter` key', () => {
    it('should be possible to open the menu with Enter', async () => {
      ...
      assertMenu({
        state: MenuState.Visible,
        attributes: { id: 'raxui-menu-items-2' },
      })
      assertMenuButtonLinkedWithMenu()
